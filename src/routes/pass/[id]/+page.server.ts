...
};mport { passes, inquiries, notifications, user } from '$lib/db/schema';
import { getDb } from '$lib/db';
import { error, fail, redirect } from '@sveltejs/kit';
import { eq, and, desc } from 'drizzle-orm';
...
export const actions = {
  createInquiry: async ({ request, params, platform, locals }) => {
    ...
    try {
      ...
      // Create notification for the pass owner (type='inquiry' for incoming request)
      await db.insert(notifications).values({
        userId: receiverUserId,
        passId: passId,
        type: 'inquiry',  // Pass owner receiving request
        title: 'New Booking Request',
        message: `New booking request from ${senderUser?.name || 'A user'} for "${pass?.title || 'a pass'}"`,
        read: false,
        archived: false,
